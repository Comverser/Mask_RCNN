...
class ParkConfig(Config):
    ...
    NUM_CLASSES = 1 + 2  # Background + 40객체
...
#         
        self.add_class("seg", 1, "Parking Space")
        self.add_class("seg", 2, "Driveable Space")
            ...
            if os.path.exists(image_path):
                try:
                    ...
                    self.add_image(
                        ...
                        polygons=polygons,
                        num_ids=num_ids)
                    seg_file_list.append(image_path)
                except :
#                     print(f'{image_path} 불러오기 실패')
                    pass
        import csv
        
        with open(f'seglistfile{subset}.csv','w') as f :
            writer = csv.writer(f)
            writer.writerow(seg_file_list)
